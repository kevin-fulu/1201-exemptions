...
| `1201/Policy Studies/` | The Register's two standalone 2016-17 policy studies (distinct from the triennial rulemakings): the 2017 §1201 study (`COLC-2015-0012`, 141 public comments) and the 2016 software-enabled-consumer-products study (`COLC-2015-0011`, 32 public comments). Each subdir holds the Full Report (`.pdf` + `.txt`), the Federal Register NOI/extension/roundtable notices, public-roundtable transcripts, and a `Comments/` directory with every public submission (inline text + attachments + `manifest.csv`). |
| `1201/Proposed Legislation/` | Draft bills we're working to get introduced (currently the *Freedom To Fix Act*, a §1201 amendment adding a permanent exemption for diagnosis / maintenance / cybersecurity research / repair of digital electronic equipment). Forward-looking artifacts, not rulemaking record. |
...
| `1201/Claude Reports/` | Long-form topic-specific research files produced in earlier sessions (smart-TV ACR, voice-assistant petitioning strategy, GE water-filter coverage, smart-appliance end-of-support strategy, the 2027-cycle multi-class petition plan, the Office policy-reports synthesis). |

Every PDF in `Final Rules/`, `Reg Recommendation/`, `first-round-comments/`, and `Policy Studies/` has a sibling `.txt` extracted with pypdf (via `tools/extract-pdf.py`) and preserving `[PAGE N]` markers that match PDF page numbers. Searching is much faster against the `.txt` files than against the PDFs. About 20 of the 2018 first-round comments and a smaller number of older policy-study comments (plus the image-only Grassley-Leahy request letter) are scanned PDFs with no embedded text layer — their `.txt` files are nearly empty by design, not by extraction bug.
